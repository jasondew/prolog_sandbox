valid_board([]). valid_board([(_, Col)|Tail]) :- member(Col, [1,2,3,4,5,6,7,8]), valid_board(Tail).
...
eight_queens(Board) :-
  ...
  cols(Board, Cols), fd_all_different(Cols),
  diags1(Board, Diags1), fd_all_different(Diags1),
  diags2(Board, Diags2), fd_all_different(Diags2).
